          ...
          switch ($fetchtype) {
            ...
            default:
              ...
              $rvalue = new Decompiler_Fetch($rvalue, $fetchtype, $globalname,
                ($opc == XC_FETCH_RW || (($opc == XC_FETCH_W || $opc == XC_FETCH_R) && !($EX['silence'] && $op1['op_type'] == XC_IS_CONST) && $op1['op_type'] != XC_IS_CONST)),$EX['silence']);
              break;
          }
          if ($opc == XC_UNSET_VAR) {
            // TODO: РїСЂРѕРІРµСЂРёС‚СЊ РєРѕСЂСЂРµРєС‚РЅРѕСЃС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС†РёР№ unset($$var)
            $op['php'] = "unset(" . ($ext != ZEND_QUICK_SET  && $op2['EA.type'] != ZEND_FETCH_STATIC_MEMBER ? '$' : '').str($rvalue, $EX) . ")";
            $lastphpop = & $op;
          }
          else {
...
//          }
        ...
        case XC_ASSIGN_OBJ:
          ...
          if ($op2val[0] == '$' && $i <= $range[1] && in_array($opcodes[$i+1]['opcode'],array(XC_ASSIGN_OBJ,XC_ASSIGN_DIM))) $op2val = '{'.$op2val.'}'; // РєРѕРЅСЃС‚СЂСѓРєС†РёРё РІРёРґР° $v->{$field}[] = 1; $v->{$field}->var = 1;
          ...
          if ($opc == XC_ASSIGN_OBJ) {
            ...
            if (is_a($rvalue, "Decompiler_DimBox")) {
              ...
              $dim->assign = $lvalue;
              if ($dim->isLast) {
                $resvar = (is_object($dim->value) && is_a($dim->value,'Decompiler_ListBox')) ? $dim->value->toObject($this,XC_ASSIGN_OBJ) : foldToCode($dim->value);
              } else unset($resvar);
              unset($dim);
              break;
            } else $resvar = new Decompiler_Binop($this, $lvalue, XC_ASSIGN_OBJ, $rvalue);
          }
          break;
        // }}}
