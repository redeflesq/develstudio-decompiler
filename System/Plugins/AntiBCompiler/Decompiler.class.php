...
////////////////////////////////////////////////////////////
//XCACHE DECOMPILER
//fixes of sidxx55, StealthDebuger, dima2k, olivok 
//see comments in code
//add you name here
/////////////////////////////////////////////////////////// 
//update log: 
//https://bitbucket.org/xdasm/decompiler/commits/all
//or http://www.nulled.cc/threads/234370/
////////////////////////////////////////////////////////////
...
function removeSlashQuotes($str)
{
    return str_replace(
        array(
            '[\"',
...
class Decompiler_Value extends Decompiler_Object // {{{
{
    ...
    function toCode($indent)
    {
        if (gettype($this->value) == 'string') {
            if ($GLOBALS['_CURRENT_FILE'] && $this->value == $GLOBALS['_CURRENT_FILE']) {
                return '__FILE__';
            }
            if ($GLOBALS['_CURRENT_FILE'] && $this->value == dirname($GLOBALS['_CURRENT_FILE'])) {
                return '__DIR__';
            }
            ...
            if ($this->noescape) {
                ...
                // TODO: РґР»СЏ PHP 5.4 РЅСѓР¶РЅРѕ РµС‰Рµ СЌСЂР°РЅРёСЂРІРѕР°С‚СЊ \e
                // print ("#".$this->value."#\n");
                // addcslashes РґРѕР±Р°РІР»СЏРµС‚ РІРјРµСЃС‚Рѕ \x8 - \b, РєРѕС‚РѕСЂС‹Р№ РЅРµ РїРѕРЅРёРјР°РµС‚ PHP
                $v = str_replace(
                    '\b', "\\010",
                    addcslashes(str_replace("\\", "@@--SLASH--@@", $this->value), "\t\n\v\f\r\"\0..\37\$")
                );
...
class Decompiler_Binop extends Decompiler_Code // {{{
{
    ...
    function toCode($indent)
    {
        ...
        if (is_a($this->op1, 'Decompiler_TriOp') || is_a(
                $this->op1, 'Decompiler_Binop'
            ) && $this->op1->opc != $this->opc) {
            $op1 = "(" . str($this->op1, $indent) . ")";
        } else {
            $op1 = $this->op1;
        }

        if (is_a($this->op2, 'Decompiler_TriOp') || is_a(
                $this->op2, 'Decompiler_Binop'
            ) && $this->op2->opc != $this->opc && substr($opstr, -1) != '=') {
        ...
        if ($this->opc == XC_CONCAT && substr($sop1, -1) == '"' && $sop2[0] == '"') {
            ...
            if (preg_match(
                    "#^([^\\\\]*?)(\\$[\\w_][\\w\\d_\\\\]*$|\\$[\\w_][\\w\\d_\\\\]*->[\\w_][\\w\\d_]*$)#", $sop1, $pr
                )
                && preg_match("#[\\w\\d_\\\\]#", $sop2[0])) {
                $sop1 = $pr[1] . '{' . $pr[2] . '}';
            }
            $this->res = $sop1 . $sop2;
        } else {
...
class Decompiler_Box // {{{
{
    ...
    {
        if (method_exists($this->obj, 'toObject')) {
            return $this->obj->toObject($parent, $assigncmd);
        } else {
            return $this->obj->toCode('');
        }
    }
}
...
class Decompiler_List extends Decompiler_Code // {{{
{
    ...
    function toCode($indent, $asobject = false, &$parent = NULL, $assigncmd = XC_ASSIGN_REF)
    {
        if (count($this->dims) == 1 && !$this->everLocked) {
            ...
            if ($asobject) {
                return new Decompiler_Binop($parent, $this->dims[0]->assign, $assigncmd, $dim);
            } else {
        ...
        if ($asobject) {
            return new Decompiler_Binop($parent, $this->toList($assigns), $assigncmd, $this->src);
        } else {
...
class Decompiler
{
    ...
    function decompileComplexBlock(&$EX, $range) // {{{
    {
        ...
        if (($firstOp['opcode'] == XC_JMPZ_EX || $firstOp['opcode'] == XC_JMPNZ_EX) && !empty($firstOp['jmpouts'])
            ...
            $this->recognizeAndDecompileClosedBlocks(
                $EX, array(
                    $range[0],
                    $range[0]
                )
        ...
        if (!empty($firstOp['jmpins']) && $opcodes[$firstOp['jmpins'][0]]['opcode'] == XC_JMP
            ...
            echo $indent, 'for (', str($initial, $EX), '; ', implode(', ', $conditionCodes), '; ', implode(
                ', ', $nextCodes
            ), ') ', '{', PHP_EOL;
            $this->beginScope($EX);
        ...
        if (!empty($firstOp['jmpins']) && !empty($opcodes[$firstOp['jmpins'][0]]['isCatchBegin'])) {
            ...
            for ($i = $catchFirst; $i <= $range[1];) {
                if ($opcodes[$i]['opcode'] == XC_CATCH) {
                    ...
                    if ($opcodes[$catchBodyLast]['opcode'] == XC_JMP && (empty($opcodes[$catchBodyLast]['jmpouts'][0]) || $opcodes[$catchBodyLast]['jmpouts'][0] != $catchNext)) {
                        --$catchBodyLast;
                    }
                    $catchBlocks[$catchFirst] = array(
                        $catchOpLine,
                        $catchBodyLast
                    );
            ...
            foreach ($catchBlocks as $catchFirst => $catchInfo) {
                ...
                echo $indent, 'catch (', str($this->getOpVal($catchOp['op1'], $EX)), ' ', str(
                    $this->getOpVal($catchOp['op2'], $EX)
                ), ") {", PHP_EOL;
                unset($catchOp);
        ...
        if (
            ...
            for ($i = $range[0]; $i <= $range[1];) {
                ...
                if ($op['opcode'] == XC_CASE) {
                    ...
                    // СѓРґР°Р»РµРЅРёРµ СЃСЃС‹Р»РѕРє СЃ РєРѕРјР°РЅРґ РѕС‡РёС‚РєРё РїРµСЂРµРјРµРЅРЅРѕР№-СѓСЃР»РѕРІРёСЏ
            ...
            } else if ($firstOp['opcode'] == XC_JMP && !empty($firstOp['jmpouts']) && $opcodes[$firstOp['jmpouts'][0]]['opcode'] == XC_JMP) {
        ...
        if (isset($firstJmpOp)
            ...
            if (isset($firstJmpOp['fe_key'])) {
                $as = str(
                        $firstJmpOp['fe_key'], $EX
                    ) . ' => ' . ($firstJmpOp['extended_value'] & ZEND_FE_FETCH_BYREF ? '&' : '') . str($as);
            } else {
                $as = ($firstJmpOp['extended_value'] & ZEND_FE_FETCH_BYREF ? '&' : '') . str(
                        $as
                    ); //StealthDebuger foreach fix code
            }
        ...
        $this->decompileBasicBlock(
            $EX, array(
                $range[0],
                $range[0]
            )
        ); //dima2k while temp fix
        $this->recognizeAndDecompileClosedBlocks(
            $EX, array(
                $range[0] + 1,
                $range[1]
                ...
                case XC_UNSET_VAR:
                    ...
                    switch ($fetchtype) {
                        ...
                        default:
                            ...
                            // Р’ СЂРµР¶РёРјРµ SILENT РёРјРµРЅСЏ РїРµСЂРµРјРµРЅРЅС‹С… РЅР°С‡РёРЅР°СЋС‚ РїРµСЂРµРґР°РІР°С‚СЊСЃСЏ РєР°Рє СЃСЃС‹Р»РєРё РЅР° СЃС‚СЂРѕРєРё-РєРѕРЅСЃС‚Р°РЅС‚С‹
                            // TODO: РїСЂРѕРІРµСЂРёС‚СЊ, РІРµР·РґРµ Р»Рё РєРѕСЂСЂРµРєС‚РЅРѕ РїСЂРѕС…РѕРґРёС‚ $$var, $$var[0] Рё С‚.Рї.
                            $rvalue = new Decompiler_Fetch(
                                $rvalue, $fetchtype, $globalname,
                                ($opc == XC_FETCH_RW || (($opc == XC_FETCH_W || $opc == XC_FETCH_R) && !($EX['silence'] && $op1['op_type'] == XC_IS_CONST) && $op1['op_type'] != XC_IS_CONST)),
                                $EX['silence']
                            );
                            break;
                    }
                    if ($opc == XC_UNSET_VAR) {
                        // TODO: РїСЂРѕРІРµСЂРёС‚СЊ РєРѕСЂСЂРµРєС‚РЅРѕСЃС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС†РёР№ unset($$var)
                        $op['php'] = "unset(" . ($ext != ZEND_QUICK_SET && $op2['EA.type'] != ZEND_FETCH_STATIC_MEMBER ? '$' : '') . str(
                                $rvalue, $EX
                            ) . ")";
                        $lastphpop = &$op;
                    } else {
              ...
              if ($list_link->obj->dims[count($list_link->obj->dims)-1]->isLast) {
                ...
                unset($list_link);
                    ...
                    if ($ext == ZEND_FETCH_ADD_LOCK) {
                        ...
                        if ($ext == ZEND_FETCH_STANDARD && (!$i || $opcodes[$i - 1]['opcode'] != XC_FETCH_DIM_TMP_VAR)) {
                            ...
                            if ($i && $opc == XC_FETCH_DIM_R && $op1['op_type'] == XC_IS_VAR && in_array(
                                    $opcodes[$i - 1]['opcode'], array(
                                        XC_DO_FCALL,
                                        XC_DO_FCALL_BY_NAME,
                                        XC_DO_FCALL_BY_FUNC,
                                        XC_EXT_FCALL_END
                    ...
                    if ($opc == XC_FETCH_DIM_TMP_VAR && $op1['op_type'] == XC_IS_TMP_VAR) {
                        for ($ti = $i + 1; $ti <= $range[1]; $ti++) {
                            if ($opcodes[$ti]['opcode'] == XC_FETCH_DIM_TMP_VAR && $opcodes[$ti]['op1']['op_type'] == XC_IS_TMP_VAR && $opcodes[$ti]['op1']['var'] == $op1['var']) {
                                break;
                            }
                    ...
                    if ($opc == XC_ASSIGN_DIM) {
                        ...
                        if (is_a($rvalue, "Decompiler_DimBox")) {
                            ...
                            $dim->assign = $lvalue;
                            if ($dim->isLast) {
                                $resvar = (is_object($dim->value) && is_a(
                                        $dim->value, 'Decompiler_ListBox'
                                    )) ? $dim->value->toObject($this, XC_ASSIGN_DIM) : foldToCode($dim->value);
                            }
                            unset($dim);
                            break;
                        }
                ...
                case XC_ASSIGN: // {{{
                    ...
                    if (is_a($rvalue, "Decompiler_DimBox")) {
                        ...
                        if ($dim->isLast) {
                            $resvar = (is_object($dim->value) && is_a(
                                    $dim->value, 'Decompiler_ListBox'
                                )) ? $dim->value->toObject($this, XC_ASSIGN) : foldToCode($dim->value);
                        }
                        unset($dim);
                        break;
                    }
                ...
                case XC_ASSIGN_REF: // {{{
                    ...
                    if (is_a($rvalue, "Decompiler_DimBox")) {
                        ...
                        $resvar = (is_object($dim->value) && is_a(
                                $dim->value, 'Decompiler_ListBox'
                            )) ? $dim->value->toObject($this, XC_ASSIGN_REF) : foldToCode($dim->value);
                        unset($dim);
                        break;
                    }
                ...
                case XC_ASSIGN_OBJ:
                    ...
                    $op2val = unquoteVariableName($this->getOpVal($op2, $EX), $EX);
                    if ($op2val[0] == '$' && $i <= $range[1] && in_array(
                            $opcodes[$i + 1]['opcode'], array(
                                XC_ASSIGN_OBJ,
                                XC_ASSIGN_DIM
                            )
                        ...
                        $op2val = '{' . $op2val . '}';
                    } // РєРѕРЅСЃС‚СЂСѓРєС†РёРё РІРёРґР° $v->{$field}[] = 1; $v->{$field}->var = 1;
                    $rvalue = str($obj) . "->" . $op2val;
                    if ($res['op_type'] != XC_IS_UNUSED) {
                        $resvar = $rvalue;
                    }
                    if ($opc == XC_ASSIGN_OBJ) {
                        ...
                        if (is_a($rvalue, "Decompiler_DimBox")) {
                            ...
                            if ($dim->isLast) {
                                $resvar = (is_object($dim->value) && is_a(
                                        $dim->value, 'Decompiler_ListBox'
                                    )) ? $dim->value->toObject($this, XC_ASSIGN_OBJ) : foldToCode($dim->value);
                            } else {
                                unset($resvar);
                            }
                            unset($dim);
                            ...
                            $resvar = new Decompiler_Binop($this, $lvalue, XC_ASSIGN_OBJ, $rvalue);
                        }
                    }
                    break;
                // }}}
                    ...
                    if ($opc == XC_ISSET_ISEMPTY_VAR) {
                        ...
                        if ($opc == XC_ISSET_ISEMPTY) {
                            ...
                            if ($opc == XC_ISSET_ISEMPTY_PROP_OBJ) {
                                ...
                                $rvalue = str($container) . "->" . unquoteVariableName($dim); //dima2k fix
                            } else {
                                $rvalue = str($container) . '[' . str($dim) . ']'; //StealthDebuger string fix
                            }
                        }
                    }
                ...
                case XC_DO_FCALL_BY_NAME: // {{{
                    ...
                    $resvar = $prefix
                        . (!$prefix ? $this->stripNamespace($fname) : (!isset($EX['called_scope']) && (!is_object(
                                $EX['fbc']
                            ) || get_class($EX['fbc']) != 'Decompiler_Value') ? '{' . $fname . '}' : $fname))
                        . "($args)";
                    unset($args);
                ...
                case XC_ADD_VAR: // {{{
                    ...
                    $sop2 = str($op2val);
                    if ($opc == XC_ADD_VAR) {
                        if (substr_count($sop2, '-') <= 1 && substr_count($sop2, '[') <= 1 && substr_count(
                                $sop2, '"'
                            ) == 0 && substr_count($sop2, '-') + substr_count($sop2, '[') <= 1) {
                ...
                // }}}
                case XC_FREE: // {{{
                    $nextopc = $opcodes[$i - 1]['opcode'];
                    if ($nextopc != XC_INIT_ARRAY && $nextopc != XC_ADD_ARRAY_ELEMENT) {
                        $free = $T[$op1['var']];
                ...
                case XC_PRE_INC_OBJ: // {{{
                    ...
                    if (isset($flags['OBJ'])) {
                        $resvar = str($this->getOpVal($op1, $EX)) . '->' . unquoteVariableName(
                                $this->getOpVal($op2, $EX)
                            );
                    } else {
                        $resvar = $this->getOpVal($op1, $EX);
                    }
    ...
    function getOpVal($op, &$EX, $free = false, $noescape = false) // {{{
    {
        switch ($op['op_type']) {
            ...
            case XC_IS_UNUSED:
                if (isset($op['EA.type']) && $op['EA.type'] == ZEND_FETCH_CLASS_GLOBAL && $op['var'] && $op['var'] == $op['opline_num'] && $op['op_num'] == 1) {
                    return '$this';
                }
                return NULL;
        }
    ...
    function dclass($class, $indent = '', &$ct = NULL) // {{{
    {
            ...
            if (!empty($class[$type])) {
                ...
                foreach ($class[$type] as $name => $v) {
                    if (isset($class['parent'])) {
                        ...
                        if (isset($p[$type]) && isset($p[$type][$name]) && $p[$type][$name] == $v) {
                            continue;
                        }
                    }
                    echo $newindent;
        ...
        if ($member_variables) {
            ...
            foreach ($member_variables as $name => $dummy) {
                ...
                if (isset($info['ce']) && $info['ce'] != $class['name']) {
                    continue;
                }
        ...
        if (isset($class['function_table'])) {
            foreach ($class['function_table'] as $func) {
                if (!isset($func['scope']) || $func['scope'] == $class['name']) {
                    ...
                    if (!isset($class['parent']) || !$this->functionExistInParent(
                            $this->findClassByName($ct, $class['parent']), $func
                        )) {
                        echo "\n";
                        $opa = $func['op_array'];
    ...
    function functionExistInParent(&$parentclass, &$fun)
    {
        if (is_array($parentclass) && is_array($fun)) {
            ...
            if (is_array(
                    $fn
                ) && $fn['op_array']['line_start'] == $fun['op_array']['line_start'] && $fn['op_array']['fn_flags'] == $fun['op_array']['fn_flags']) {
                return true;
            }
        }
        return false;
    ...
    function &findFunctionByName(&$ft, $name)
    {
        foreach (array_keys($ft) as $n) {
            if ($ft[$n]['op_array']['function_name'] == $name) {
                return $ft[$n];
            }
        }
        $r = NULL;
    ...
    function &fixOpcode($opcodes, $removeTailing = false, $defaultReturnValue = NULL) // {{{
    {
        ...
        if ($removeTailing) {
            ...
            if ($opcodes[$last]['opcode'] == XC_RETURN) {
                ...
                if ($op1['op_type'] == XC_IS_CONST && array_key_exists(
                        'constant', $op1
                    ) && $op1['constant'] === $defaultReturnValue) {
                    $opcodes[$last]['opcode'] = XC_NOP;
                    --$last;
                }
            }
    ...
    function dargs(&$EX) // {{{
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if (isset($EX['recvs'][$i + 1])) {
                $arg = $EX['recvs'][$i + 1];
            } else {
                $arg = array();
            } // fix alert for $isInterface || $isAbstractMethod
